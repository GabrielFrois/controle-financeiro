...
export default function Investments() {
  ...
  const stats = useMemo(() => {
    ...
    investTrans.forEach(t => {
      ...
      if (isRF) {
        ...
        rendaFixaItems.push({
          ...
          totalCost: val,
          avgPrice: val,
          currentPrice: valorAtualizado,
          currentTotal: valorAtualizado,
          profitLoss: valorAtualizado - val,
          performance: val > 0 ? ((valorAtualizado / val) - 1) * 100 : 0,
          isRF: true,
          type: 'RENDA_FIXA'
        });
      } else if (t.asset_ticker) {
        // --- Lógica de Renda Variável ---
    ...
    // Processa posições de Renda Variável
    ...
    [...investTrans].sort((a, b) => a.date.localeCompare(b.date)).forEach(t => {
      const monthYear = t.date.substring(0, 7);
      if (!monthlyMap[monthYear]) monthlyMap[monthYear] = { month: monthYear, dividendos: 0, patrimony: 0 };
      const val = Number(t.amount || 0);
      if (t.category_name.includes('Aporte')) monthlyMap[monthYear].patrimony += val;
      if (t.category_name.includes('Resgate')) monthlyMap[monthYear].patrimony -= val;
    ...
    return {
      ...
      fullHistory: Object.values(monthlyMap).map((item: any) => ({
        ...item,
